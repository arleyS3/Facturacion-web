...
public class TramaController {
    ...
    @Operation(summary = "Generar trama TXT (en JSON)", description = "Genera la trama TXT a partir del payload por secciones (campos + listas). "
            +
    ...
    @ApiResponses({
            ...
            @ApiResponse(responseCode = "400", description = "Request inválido o validación de negocio fallida", content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = ErrorResponse.class)))
    })
    ...
    public GenerarTramaResponse generar(
            @io.swagger.v3.oas.annotations.parameters.RequestBody(required = true, content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE, examples = {
                    @ExampleObject(name = "Factura (01) - exonerada simple", description = "Factura con 1 item exonerado (CodigoTipoIgv=9998).", value = TramaExample.FACTURA_EXONERADA_SIMPLE),
                    @ExampleObject(name = "Boleta (03) - alias tipoDocumento", description = "tipoDocumento puede enviarse como alias 'Boleta'.", value = TramaExample.BOLETA_ALIAS),
                    @ExampleObject(name = "Nota de Crédito (07) - esquema base", description = "Mismo payload base por secciones; reglas específicas de 07 se irán ajustando conforme a necesidad.", value = TramaExample.NOTA_CREDITO_BASE),
                    @ExampleObject(name = "Nota de Débito (08) - esquema base", value = TramaExample.NOTA_DEBITO_BASE),
                    @ExampleObject(name = "Guía de Remisión (09) - mínima", description = "Mínimo para GR: sección A común + sección G con motivo/modalidad. Secciones adicionales se envían según el caso.", value = TramaExample.GUIA_REMISION_MINIMA)
    ...
    @Operation(summary = "Descargar trama TXT (.txt)", description = "Genera la trama y la devuelve como archivo de texto (Content-Disposition: attachment). Ideal para botón 'Descargar'.")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Archivo TXT generado", content = @Content(mediaType = MediaType.TEXT_PLAIN_VALUE)),
            @ApiResponse(responseCode = "400", description = "Request inválido o validación de negocio fallida", content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = ErrorResponse.class)))
    })
    ...
    @Operation(summary = "Importar trama TXT (.txt) y convertir a JSON por secciones", description = "Recibe un archivo TXT (multipart/form-data) y devuelve el tipo de documento detectado y el payload por secciones (campos + listas).")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Trama importada"),
            @ApiResponse(responseCode = "400", description = "Archivo inválido", content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = ErrorResponse.class)))
    })
